...
jetson-arena vendors its `.claude/skills/` from **guildmaster** — the
...
#   - consumer-identifying prose: `guildmaster` → `jetson-arena` (NOT
...
If a re-sync would lose a jetson-arena adaptation, lift the change
...
#     → `which jetson-arena vendors from guildmaster`
