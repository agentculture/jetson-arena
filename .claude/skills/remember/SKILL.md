...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope jetson-arena
...
scope:** the default personal scope is **private** (`--scope jetson-arena
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `jetson-arena`). Private records are served only to a recall in the same
