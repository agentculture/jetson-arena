...
type: command
description: >
  Cross-repo + mesh communication from jetson-arena: file tracked GitHub issues
  ...
  the next step lives outside jetson-arena (a brief for a sibling-repo agent, a
  status ping for a Culture channel, or pulling an issue body + comments
  into context). Issue posts auto-sign with `- jetson-arena (Claude)`; mesh
  messages are unsigned (the IRC nick is the speaker). Not for in-jetson-arena
